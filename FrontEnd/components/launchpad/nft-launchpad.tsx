...
const rarities: Rarity[] = [
  ...
  { name: "Rare", color: "text-blue400", supply: 100, minted: 0 },
  { name: "Ultra Rare", color: "text-purple300", supply: 50, minted: 0 },
  { name: "Legendary", color: "text-yellow400", supply: 26, minted: 0 },
  { name: "Mythic", color: "text-red400", supply: 4, minted: 2 },
]
...
export default function NFTLaunchpad({ collectionName }: NFTLaunchpadProps) {
  ...
  const currentNetwork =
    chainIdFromWallet === 84532
      ? "base"
      : chainIdFromWallet === 43113
      ? "avalanche"
      : "monad"
  ...
  const isSoldOut = () => {
    const selectedRarityData = dynamicRarities.find(
      (rarity) => rarity.name === selectedRarity
    )
    return selectedRarityData
      ? selectedRarityData.minted >= selectedRarityData.supply
      : false
  }
  ...
  const handlePriceDisplay = () => {
    if (hasRarities && collectionPrices) {
      return (
        collectionPrices[currentNetwork]?.[selectedRarity] ?? {
          crypto: "0",
          fiat: "$0",
        }
      )
  ...
  useEffect(() => {
    setEndTime(new Date(Date.now() + 90 * 24 * 60 * 60 * 1000))
  }, [])

  useEffect(() => {
    async function fetchCombinedRarityStats() {
      try {
        const baseChainId = 84532 // Base Sepolia
        const avaxChainId = 43113 // Avalanche Fuji
  
        const baseSubgraphUrl = getSubgraphUrl(baseChainId)
        const avaxSubgraphUrl = getSubgraphUrl(avaxChainId)
  
        const rarityIds = ["0", "1", "2", "3", "4"]
  
        const combinedStatsPromises = rarityIds.map(async (id) => {
          const [baseRes, avaxRes] = await Promise.all([
            request<RarityStatResponse>(baseSubgraphUrl, GET_RARITY_STATS, { id }),
            request<RarityStatResponse>(avaxSubgraphUrl, GET_RARITY_STATS, { id }),
          ])
  
          const baseMinted = baseRes?.rarityStats?.minted ?? 0
          const avaxMinted = avaxRes?.rarityStats?.minted ?? 0
          const totalMinted = Number(baseMinted) + Number(avaxMinted)
  
          return { rarityId: id, minted: totalMinted }
        })
  
        const combinedStats = await Promise.all(combinedStatsPromises)
  
        const updatedRarities = rarities.map((rarity, index) => {
          const combinedStat = combinedStats.find(
            (stat) => Number(stat.rarityId) === index
          )
          return {
            ...rarity,
            minted: combinedStat ? combinedStat.minted : rarity.minted,
          }
        })
  
        setDynamicRarities(updatedRarities)
        console.log("Combined updated rarities:", updatedRarities)
      } catch (error) {
        console.error("Error fetching combined rarity stats:", error)
      }
    }
  
    fetchCombinedRarityStats()
  }, [mintCount])
  ...
  return (
    <div className="relative bg-gradient-to-bl from-blue/80 via-black/70 to-blue/80 text-white pb-10 pt-3 md:pt-[30px] md:pb-[74px]">
      ...
      <div className="px-8 md:px-0 max-w-6xl mx-auto grid md:grid-cols-2 gap-6">
        ...
        <div className="space-y-4 mt-4 z-10">
          <div className="flex text-start justify-start items-start">
            <div className="flex gap-2">
              <span className="text-white font-bold text-xl md:text-3xl mr-1 md:mr-2">
                {title}
              </span>
            ...
            {images.map((image, index) => (
              <button
                ...
                className={`relative aspect-square rounded-xl overflow-hidden border-2 transition-all ${
                  selectedImage === index
                    ? "border-white"
                    : "border-transparent hover:border-purple600/50"
                }`}
              >
                <Image
                  src={image}
        ...
        <div className="space-y-6 z-10">
            ...
            <div className="space-y-6">
              {mintStages?.map((stage, index) => (
                <div
                  ...
                  className={`space-y-2 ${
                    stage.status === "LIVE"
                      ? "p-4 border border-purple rounded-xl"
                      : "p-4 border border-gray-700/40 rounded-xl"
                  }`}
                >
          ...
          <div className="bg-[#18181c] p-4 rounded-xl space-y-6">
            <Button
              variant="outline"
              className="w-full bg-[#26282d] border-none hover:bg-[#2a2b30] z-10"
            >
              Explore Collection
      ...
      {/* NFT Mint Popup displayed on successful mint */}
      {showPopup && (
        <NFTMintPopup
          isOpen={showPopup}
          onClose={() => setShowPopup(false)}
          collectionKey={collectionName.toLowerCase()}
        />
      )}
    </div>
  )
