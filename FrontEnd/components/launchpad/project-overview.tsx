...
export default function ProjectOverview({ collectionName}: ProjectOverviewProps) {
    ...
    const config = collectionName && collectionConfig[collectionName.toLowerCase()];
    if (!config) {
    return (
  ...
  const { title, contractAddress, milestones, teamMembers, socials, about, utility } = config;
          ...
          <TabsContent value="overview" className="grid md:grid-cols-2 gap-12 pb-8 md:pb-0">
            ...
            <div className="space-y-8">
              <div>
                ...
                >
                Contract {contractAddress.slice(0, 5)}...{contractAddress.slice(-4)}
